...
class adcirc:
    ...
    def global_water(global_path,netcdf_file,title,hours1,levels,lon1,lon2,lat1,lat2,start=None,begin=0):
        ...
        for i in range(begin,hours1):
            ...
            cb.set_label('MSL (m)',fontsize=10)
            plt.title(title + '\n')
            plt.xlabel('\nDate:{}'.format(start_date+ timedelta(hours=i/2)))
    ...
    def global_wind_mag(global_path,netcdf_file,title,hours,levels,lon1,lon2,lat1,lat2,start=None,begin=0):
        ...
        for i in range(begin,hours):
            ...
            plt.xlim([lon1, lon2])
            plt.ylim([lat1, lat2])    
            plt.tricontourf(triang,mag, levels=levels,alpha=0.9,vmin=np.min(levels), vmax=np.max(levels), aspect='auto',cmap='ocean')
            file_number = '%05d'%i
